...
import { Config, configSchema, defaultConfig } from '../types/config.js';
 ...
 */
function expandConfigPaths(config: Config): void {
 ...
 */
function decryptCredentials(config: Config, encryptionKey: string): void {
 ...
 */
function encryptCredentials(config: Config, encryptionKey: string): Config {
...
export function loadConfig(configPath?: string): Config {
  ...
  // Expand paths
  expandConfigPaths(config as Config);
  
  // Decrypt credentials using the encryption key
  decryptCredentials(config as Config, config.storage.encryptionKey);
...
export function saveConfig(config: Config, configPath?: string): void {
  ...
  try {
    ...
    throw new ConfigurationError(
      `Failed to save configuration to ${resolvedPath}: ${(error as Error).message}`
    );
  }
}

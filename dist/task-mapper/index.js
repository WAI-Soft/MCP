...
import { kiroTaskSchema, cirvoyTaskSchema } from '../types/task';
/**
...
export class TaskMapper {
     ...
     */
    createMapping(kiroPath, cirvoyId, specId, lastSyncDirection = 'kiro-to-cirvoy') {
        try {
            this.insertStmt.run(kiroPath, cirvoyId, specId, new Date().toISOString(), lastSyncDirection, 1 // Initial version
            );
        }
        catch (error) {
     ...
     */
    getMapping(kiroPath) {
        const row = this.selectByKiroPathStmt.get(kiroPath);
        if (!row) {
            return null;
        }
        return {
     ...
     * Uses prepared statement and indexed query for optimal performance
     */
    getMappingByCirvoyId(cirvoyId) {
        const row = this.selectByCirvoyIdStmt.get(cirvoyId);
        if (!row) {
            return null;
        }
        return {
     ...
     */
    updateSyncMetadata(kiroPath, direction) {
        const stmt = this.db.prepare(`
      UPDATE task_mappings
      SET 
        last_synced_at = ?,
     ...
     * Requirement 11.4: Preserve Unicode characters in all text fields
     */
    toCirvoyTask(kiroTask, projectId) {
     ...
     */
    cirvoyStatusToKiro(cirvoyStatus) {
        const statusMap = {
 ...
 */
export function createTaskMapper(db) {

...
export class OfflineQueue {
    constructor(db, retryConfig = DEFAULT_RETRY_CONFIG) {
        this.db = db;
    ...
    `);
        const result = stmt.run(update.operation, update.direction, JSON.stringify(update.taskData), update.retryCount ?? 0, update.maxRetries ?? 3, update.nextRetryAt ? update.nextRetryAt.toISOString() : null, update.status ?? 'pending');
     ...
     */
    getQueuedUpdates(status = 'pending', limit) {
        let query = `
      SELECT 
        id,
        operation,
 ...
 */
export function createOfflineQueue(db, retryConfig) {

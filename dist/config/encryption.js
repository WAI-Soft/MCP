...
function validateKey(key) {
    if (key.length !== KEY_LENGTH) {
        throw new EncryptionError(`Encryption key must be exactly ${KEY_LENGTH} characters (256 bits). ` +
...
export function decrypt(encrypted, key) {
    try {
        ...
        if (parts.length !== 3) {
            throw new EncryptionError('Invalid encrypted format. Expected format: iv:authTag:ciphertext');
        }
